...
void troca(int *a, int *b) {
  /*
  troca valores para percorrer o tamanho da taxa de variação entre o maior e o
  menor para a função cg_drawline. Permitindo assim chamar a função com pontos
  distintos P(x1,y1) e Q(x2,y2) sem distinção de ordem entre qual é o maior e o
...
void cg_drawcircle(float r, int cx, int cy, char *s) {
  int i, p, x1, y1, y, x;
  float theta;
  for (i = 0; i <= 360; i++) {
    // printf("i: %d p(%d, %d)\n", i, x, y);
    theta = (i * 3.14) / 180;
    x = (int)round((cx + (r * cos(theta))));
    y = (int)round((cy + (r * sin(theta))));
    term_move_xy(x, y);
    printf("@", s);
  }
}
...
void cg_drawline(int x1, int y1, int x2, int y2) {
  ...
  onde -a/b = (y2-y1)/(x2-x1) = m -> coeficiente angular
       -c/b = [(x2y1)-(x1y2)]/x2-x1 = q -> coeficiente linear
           ::\> y = mx+q
  */
  float a = y1 - y2, b = x2 - x1, c = (x1 * y2) - (x2 * y1), x = 0, y = 0;
